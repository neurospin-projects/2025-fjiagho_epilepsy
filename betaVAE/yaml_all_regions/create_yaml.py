...
def replace_reference_yaml(crop_dir, side, ref):
  ...
  if not os.path.exists(mask_file):
      print(f"⚠️  Fichier manquant : {mask_file} — ignoré.")
      return None, None  # renvoie None pour signaler qu’il faut passer au suivant

  try:
      with open(mask_file, 'r') as file:
          mask = file.read()
  except Exception as e:
      print(f"⚠️  Erreur en lisant {mask_file} : {e}")
      return None, None
...
for crop_dir in crop_dirs:
    for side in ['L', 'R']:
        ...
        # Si le fichier est manquant ou erreur → on saute
        if filedata is None or result_file is None:
            continue  
